...
const formatOrderImagePaths = (order) => {
  order.payment_proof_image = formatImagePath(order.payment_proof_image);

  if (order.items && Array.isArray(order.items)) {
    order.items.forEach((item) => {
      item.product_image = formatImagePath(item.product_image);
      item.image_url = formatImagePath(item.image_url);
    });
  }
  return order;
};
...
exports.uploadPaymentProof = (req, res) => {
  ...
  const paymentProofFilename = req.file.filename;
  const sql =
    "UPDATE orders SET payment_proof_image = ?, order_status = 'verifying_payment' WHERE order_code = ? AND id_user = ?";
  db.query(sql, [paymentProofFilename, orderCode, id_user], (err, result) => {
    if (err)
    ...
    res.status(200).json({
      ...
      imageUrl: formatImagePath(paymentProofFilename),
    });
  });
};
...
exports.getUserOrders = (req, res) => {
  ...
  db.query(sql, [id_user], (err, results) => {
    ...
    const orders = Object.values(ordersMap).map(formatOrderImagePaths);
...
exports.cancelOrder = (req, res) => {
  ...
  db.beginTransaction((err) => {
    ...
    db.query(getItemsSql, [orderId], (err, items) => {
      ...
      const restoreStockPromises = items.map(
        (item) =>
          new Promise((resolve, reject) => {
...
exports.getPendingOrders = (req, res) => {
  ...
  const sql = `
    SELECT o.id_order, o.order_code, o.order_date, o.total_amount, o.order_status, o.payment_proof_image, u.name AS user_name, u.email AS user_email, u.no_handphone AS user_phone, addr.alamat_lengkap, addr.kota, addr.provinsi, addr.kode_pos, GROUP_CONCAT(JSON_OBJECT('product_name', p.nama, 'quantity', oi.quantity, 'size', oi.size, 'price', oi.price_at_purchase, 'image_url', p.image_url, 'grade', g.nama_grade, 'category', c.name, 'description', p.deskripsi)) AS items
    FROM orders o JOIN user u ON o.id_user = u.id_user JOIN user_addresses addr ON o.id_address = addr.id_address JOIN order_items oi ON o.id_order = oi.id_order JOIN produk p ON oi.id_produk = p.id_produk LEFT JOIN grades g ON p.id_grade = g.id_grade LEFT JOIN category c ON p.id_cat = c.id_cat
    WHERE o.order_status IN ('verifying_payment', 'processing')
    GROUP BY o.id_order ORDER BY o.order_date ASC;`;
  db.query(sql, (err, results) => {
    if (err)
      return res.status(500).json({ message: "Database error", error: err });
    const orders = results.map((order) => {
      const parsedOrder = { ...order, items: JSON.parse(`[${order.items}]`) };
      return formatOrderImagePaths(parsedOrder);
    });
    res.status(200).json(orders);
  });
};
...
exports.getAllOrders = (req, res) => {
  ...
  const sql = `
    SELECT o.id_order, o.order_code, o.order_date, o.total_amount, o.order_status, o.payment_proof_image, u.name AS user_name, u.email AS user_email, u.no_handphone AS user_phone, addr.alamat_lengkap, addr.kota, addr.provinsi, addr.kode_pos, GROUP_CONCAT(JSON_OBJECT('product_name', p.nama, 'quantity', oi.quantity, 'size', oi.size, 'price', oi.price_at_purchase, 'image_url', p.image_url, 'grade', g.nama_grade, 'category', c.name, 'description', p.deskripsi)) AS items
    FROM orders o JOIN user u ON o.id_user = u.id_user JOIN user_addresses addr ON o.id_address = addr.id_address JOIN order_items oi ON o.id_order = oi.id_order JOIN produk p ON oi.id_produk = p.id_produk LEFT JOIN grades g ON p.id_grade = g.id_grade LEFT JOIN category c ON p.id_cat = c.id_cat
    GROUP BY o.id_order ORDER BY o.order_date DESC;`;
  db.query(sql, (err, results) => {
    if (err)
      return res
        .status(500)
